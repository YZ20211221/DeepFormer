...
def visualize_roc_curves(prediction,
    ...
    plt.savefig(os.path.join(output_dir, "roc_curves.svg"),format="svg",dpi=dpi)
...
def visualize_precision_recall_curves(
    ...
    for index, feature_preds in enumerate(prediction.T):
        ...
        plt.step(recall, precision,alpha=0.3, lw=1, where="post")
    plt.xlim([0.0, 1.0])
    plt.ylim([0.0, 1.05])
    plt.xlabel('Recall')
    ...
    if fig_title:
        plt.title(fig_title)
    plt.savefig(os.path.join(output_dir, "Feature precision-recall curves.png"))
    plt.savefig(os.path.join(output_dir, "Feature precision-recall curves.svg"),format="svg",dpi=dpi)
